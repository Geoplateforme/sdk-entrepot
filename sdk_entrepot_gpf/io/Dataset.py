...
class Dataset:
        ...
        for _, p_elt, b_is_dir, p_md5_suf in l_md5_targets:
            ...
            if not p_md5_suf.exists():
                ...
                with open(p_md5_suf, "w", encoding="utf-8") as o_md5_file:
                    for p_file, s_md5 in self.__build_md5_content(p_elt, b_is_dir).items():
    ...
    def __resolve_data_path(self, p_dir: Path) -> Path:
        """Résout et valide un chemin déclaré dans data_dirs."""
    ...
    def __get_md5_path(self, p_dir: Path, b_is_dir: bool) -> Path:
        """Calcule le chemin local du fichier md5 associé à une entrée data_dirs."""
        p_md5 = self.__root_dir / p_dir
        if b_is_dir:
            return p_md5.with_suffix(".md5")
        return Path(f"{p_md5}.md5")
