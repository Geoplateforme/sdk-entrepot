        ...
        for d_rep in l_rep:
            ...
            with patch.object(ApiRequester, "route_request", return_value=o_response) as o_mock_request:
                with patch.object(ApiRequester, "range_next_page", return_value=False) as o_mock_range:
                    ...
                    s_data_recupere = o_log_interface.api_logs_pages_filter()
    ...
    def test_api_logs_filter(self) -> None:
        ...
        with patch.object(ApiRequester, "route_request", return_value=o_response) as o_mock_request:
            with patch.object(ApiRequester, "range_next_page", return_value=False) as o_mock_range:
                ...
                s_response_succes = s_data.split("\n")[1]
                o_log_interface = LogsInterface({"_id": "id_entité"}, datastore=d_rep["datastore"])
                s_data_recupere_info = o_log_interface.api_logs_pages_filter(str_filter="INFO")
                s_data_recupere_error = o_log_interface.api_logs_pages_filter(str_filter="ERROR")
                s_data_recupere_succes = o_log_interface.api_logs_pages_filter(str_filter="succès")
                # on vérifie que route_request et range_next_page sont appelés correctement
                ...
                self.assertEqual(s_response_succes, "\n".join(s_data_recupere_succes))
    ...
    def test_api_logs_pages_filter(self) -> None:
        ...
        with patch.object(ApiRequester, "route_request", return_value=o_response) as o_mock_request:
            with patch.object(ApiRequester, "range_next_page", return_value=False) as o_mock_range:
                ...
                s_response_succes = s_data.split("\n")[1]
                o_log_interface = LogsInterface({"_id": "id_entité"}, datastore=d_rep["datastore"])
                s_data_recupere_info = o_log_interface.api_logs_pages_filter(1, 1, 2, "INFO")
                s_data_recupere_error = o_log_interface.api_logs_pages_filter(2, 3, 2, "ERROR")
                s_data_recupere_succes = o_log_interface.api_logs_pages_filter(str_filter="succès")
                # on vérifie que route_request et range_next_page sont appelés correctement
                o_mock_request.assert_called_with(
                    "store_entity_logs",
